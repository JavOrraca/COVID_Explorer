...
COVID_cumsum <- df_details %>% 
  ...
  rename(Country_Code = country,
         ...
         Death = deaths,
         Tested = tested) %>% 
  left_join(iso3166, by = c("Country_Code" = "a3")) %>% 
  filter(is.country == "TRUE") %>% 
  select(-a2, -uncode, -is.country) %>% 
...
test <- df_details %>%
  filter(country == "USA" | country == "United States") %>% 
  distinct(name, level, state, country, aggregate)
...
plot_ly(head(COVID_totals, n = 25),
  ...
  layout(xaxis = list(title = "Infected per Day",
         ...
         yaxis = list(title = "",
                      dtick = 1)) %>% 
  add_annotations(x = ~Infected_per_Day_since_100 + 340,
...
```{r}
plot_ly(data = COVID_cumsum_USA_confirmed_hist_and_pred,
        x = ~Date,
        y = ~Values,
        name = ~State,
        type = "scatter",
...
Rolling Averages (US & California)
======================================================================
...
### **3-day Rolling Average of new Coronavirus Cases by California county**
